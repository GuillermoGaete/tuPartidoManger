...
{
    QByteArray recivedArray = m_reply->readAll();
    QVariant statusCode = m_reply->attribute( QNetworkRequest::HttpStatusCodeAttribute );
    int status = statusCode.toInt();
    emit dataRecived(recivedArray,status);
    m_retries = 0;
}
 ...
 * @date 21 Jan 2019
 * @brief Clears cache QNAM instance
 * @return void
 * @package Network
 * @public
 * Clears the QNetworkAccessManager instance cache.
 */
void Network::Manager::clearConnectionCache()
{
    m_QNAM->clearConnectionCache();
}
